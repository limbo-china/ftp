...
#define DEFAULT_HOME_DIR    "E:\\server\\"
...
void main(void)
{
   ...
   if ((sListen = WSASocket(AF_INET, SOCK_STREAM, 0, NULL, 0,		//监听端口
   ...
   if (bind(sListen, (PSOCKADDR) &inetAddr, sizeof(inetAddr)) == SOCKET_ERROR) //绑定端口号
   {
      printf("错误：bind() failed with error %d\n", WSAGetLastError());
      return;
   }

   if (listen(sListen, SOMAXCONN))		//开始监听，监听队列长度为65535
   {
   ...
   //创建第一个手动重置对象 
   if ((g_events[0] = WSACreateEvent()) == WSA_INVALID_EVENT)	//创建事件
   {
   ...
   {
       //处理入站连接
      if ((sAccept = accept(sListen, NULL, NULL)) == INVALID_SOCKET)//返回请求队列最前的客户请求
      {
...
int	DataConn( SOCKET& s, DWORD dwIp, WORD wPort, int nMode ) 
{
	...
	else if( MODE_PORT == nMode ) 
	{
		...
		{
			printf("Failed to connect a socket ,port %d ,error %d\n", wPort,WSAGetLastError() );
			closesocket( s );
			return -1;
		}
	}
...
int DealCommand( LPSOCKET_INF pSI )
{
	...
	const char*  szOpeningAMode = "151 Opening ASCII mode data connection for ";
	...
	if( strstr( szCmd, "NLST") || strstr( szCmd,"LIST") ) 
	{
		...
		if( !bPasv )
			...
			strcpy(pSI->buffSend,"150 Data connection already open; Transfer starting.\r\n");		
